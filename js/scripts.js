...
$(function () {
    ...
    gallery_1.ev.on('rsBeforeAnimStart', function (event) {
        ...
        var catalog = $('#catalog');
        console.log(target.currSlide.image)
        catalog.css({'background': 'url("'+ target.currSlide.image +'") no-repeat scroll center top #071217','background-size':'100%'});
        gallery_apocalisis(target);
    });
    ...
    function gallery_apocalisis(target) {
        ...
        var thumb = $('.rsThumbsHor')
        thumb.css("width", slideObj_width + "px");

        if (thumb.hasClass('visible')) {
...
$(window).load(function () {
    ...
    gallery_1.prepend(gallery_1.find('.rsNav'));
    var $thumbs = $('.rsThumbsHor');
    $thumbs.hide();
